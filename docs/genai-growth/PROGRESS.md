...
**2026-08-09, continued again — Olga corrected the actual priority: visual duplicates matter, name duplicates don't.** Full rationale in `DECISIONS.md` ADR-009's final 2026-08-09 update. `DESIGN_DUPLICATE_THRESHOLD`'s hard reject (above) softened back to informational — a real risk of discarding a genuinely different design over caption-similarity alone, since the check never looked at the actual picture. Added the check that was actually missing: `findNearestImageMatch()` (`semantic-search.ts`), comparing a real generated image against the catalog's `imageVec` index (reusing the same index `imageSearch()` already serves reverse-image search from, just never wired into dedup). Runs post-generation, not during `detectTrend()` (no image exists yet at that point). First real end-to-end use: generated + visually checked + saved two genuinely-new candidate themes from the "disco ball" run's other checked candidates — "Disco Ball" (pattern `01c8c44d-f199-4047-b48d-ca94ff1f072c`, 58x63, 14 colors) and a manually-authored "Minimalist Line Art Face" (pattern `58d6df0c-07c1-4774-b7ff-94980f3c0dc5`, 75x58, 9 colors — not a `detectTrend()`-researched theme, one of the other candidates it checked via `search_catalog` mid-run but didn't settle on; imagePrompt/colorPalette hand-written in the same style conventions since the model never fully elaborated it). Both scored 0.49-0.55 against clearly-unrelated existing designs — no real visual duplicate found yet to calibrate a threshold against.
Image-generation provider comparison reached its 5-round threshold this session (`IMAGE_GENERATION_PREFERENCES.md`): OpenAI 5-0, same stated criterion every round (sharper, no background-removal needed) — treated as a settled default going forward, Stability no longer needs a side-by-side run every time.

**2026-08-09, one more real production bug — Olga caught it live: the saved "Minimalist Line Art Face" draft rendered almost entirely empty on the actual site.** Took three real rounds to actually fix, each one caught by Olga pushing back on an incomplete fix rather than accepting "looks fine" — full step-by-step tracing (rendering the output after each pipeline stage, never guessing) throughout.

*Round 1.* `save-ai-draft.ts` had never called `image-analysis.ts`'s `analyzeImage()` at all — `mode` was hardcoded to `'illustration'` regardless of the source image. Traced the missing outline to `detectBackgroundByFloodFill()`/`eraseBackground()`, the file's own documented "harmless no-op for a real-alpha source" fallback — except this particular OpenAI output had `hasAlpha: false` (unlike other rounds' images), so the fallback ran for real and erased 9765/10000 cells (97.65%). Root mechanism: the tolerance-30 flood fill chained through several intermediate anti-aliased grey shades along the thin line's soft edges (5 distinct greys ended up in the palette) and tunneled from the white border all the way through to touch nearly everything, even though black and white themselves are far outside any single tolerance hop. Fixed two things: wired in `analyzeImage()`/`imageTypeToMode()` for real mode selection (classified `typography`/high confidence → `line-art` — correct, though it doesn't change `convertImage()`'s own behavior since that file treats `'illustration'`/`'line-art'` identically everywhere), and skipped the flood-fill fallback entirely for line-art/typography. Re-saved: 75x58 (heavily eroded) → 102x102 (full outline intact) — looked correct.

*Round 2 (Olga's catch).* Asked directly whether the line-art check had actually run, then asked about the flood-fill skip's real cost — checked the actual saved palette (not just the render) and found "blanc White" occupying 9175/10404 cells (88%), a huge chunk of the canvas counted as a real stitchable color instead of blank fabric, because skipping erasure entirely left the whole background as a literal white DMC entry. Olga's proposed real fix, correct: merge anti-aliased grey edge pixels into pure black/white FIRST (new `mergeGrayscaleTowardBlackWhite()`, `pattern-converter.ts`, gated to `mode === 'line-art'` only — real illustration/photo content can have legitimate grey, e.g. fur or stone, that must not be forced to black/white), then run background-erasure with EXACT match (tolerance 0) instead of skipping — a genuinely uniform background can't gradient-tunnel since no intermediate shade survives to hop through.

*Round 3.* Re-ran — tolerance-0 erasure STILL ate the line (checked directly: 557 of 653 black cells marked "background," which should have been impossible at exact-match if the seed itself were color-checked). Traced the actual flood-fill algorithm line by line and found a real, previously-hidden bug, independent of tolerance entirely: the border-seeding loop marked EVERY border cell as a background seed unconditionally, with no color check. This portrait's hairline/neck touch the frame edge — a BLACK border cell got seeded, and the walk then correctly only propagates through color-matching neighbors, but since it started ON the black stroke it consumed the whole connected outline (one continuous curve, so most of it). Fixed: a border cell now only seeds the fill if its own color is close to white (a generous 40-tolerance seed check, independent of the caller's propagation tolerance) — a black-touching-the-border case can no longer become a seed at all.

Final verified result: 80x102, 4 real colors (black outline + turquoise eye + coral lips + gold earring), full outline intact, no spurious background color, visually confirmed clean. This bug is confined to `save-ai-draft.ts`'s own `detectBackgroundByFloodFill()`/`eraseBackground()` — confirmed these aren't shared with the real editor (`ConvertClient.tsx` has its own, unrelated single-seed-point `floodFill()` for the paint-bucket tool), so real users importing photos through the live site were never affected by any of these three bugs.

**2026-08-09, same session — three more real findings from a genuine test image, Olga's own kitten illustration (`TestImages/Kitten.png`, copied in for regression-testing going forward alongside puppy/Lady of Perpetual Love).**

1. **Removed the `mode === 'photo' ? 'illustration' : mode` override** added earlier the same day in `save-ai-draft.ts`. Made sense when this script only ever processed detectTrend()-sourced AI illustrations (a safety net against the classifier saying 'photo' on illustration content), but now that it also handles Olga's own arbitrary image imports, unconditionally forcing every real photo through illustration-mode outline-detection is risky — real photographic noise/texture isn't what that system is tuned for. Olga's call: trust the classifier everywhere, including a real 'photo' verdict; added a `MODE_OVERRIDE` 8th CLI arg for the cases where the classifier itself is wrong, instead of silently overriding one outcome for every caller.

2. **`analyzeImage()` misclassified the kitten** (a genuine flat cel-shaded illustration, clearly built for exactly what the outline-preservation system targets) as `typography`. Root cause, traced through the actual heuristic: `bimodalFraction > 0.65` (true — huge white background + grey stripes/outlines) and `meanSaturation < 25` (true — the colored regions, pink ears/green eyes, are a small fraction of total area, so they don't pull the AVERAGE saturation above the illustration threshold) routes into the "bimodal + near-grayscale → line-art or typography" branch; `edgeDensity > 0.25` (true — the dense white-keyline network reads as high edge density, the same signal real text has) then picks `typography` over `line-art`. Not fixed — a real classifier retune, deferred; `MODE_OVERRIDE` (above) is the workaround for now.

3. **Real bug in the outline-detection system itself, independent of (1)/(2) and worth fixing regardless** — even with mode correctly forced to `'illustration'`, the kitten's outline still rendered broken/fragmented. Traced to the true root cause by rendering the raw top-hat magnitude as a heatmap image (not guessing): a single continuous keyline's contrast varies hugely depending on what it borders — strong against dark grey fur, weak-to-nothing against pale cream fur — so a single global `OUTLINE_TOPHAT_THRESHOLD` (50) necessarily fragments it wherever it crosses a low-contrast stretch. Simply lowering the threshold globally was already ruled out by prior tuning (25 caught real strokes but also false-positived on soft internal shading — see `OUTLINE_QUANTIZE_COLORS`'s history). Real fix: hysteresis thresholding, the same two-threshold edge-linking technique Canny edge detection uses — `OUTLINE_TOPHAT_THRESHOLD` (50) still gates which pixels can SEED a stroke (no new false positives on flat regions, unchanged), but a new `OUTLINE_TOPHAT_LOW_THRESHOLD` (15) now extends an already-seeded stroke through low-contrast stretches via 8-connected flood-fill from confirmed seeds — isolated low-contrast noise with no strong seed nearby anywhere still never qualifies on its own. Re-verified against puppy and "Lady of Perpetual Love" (both intact, no regression) before considering this safe. This lives in `pattern-converter.ts` — a real, shared-library improvement, not confined to the AI-draft pipeline; benefits any real user selecting Illustration/Line Art mode on the live site too.

Verified: `tsc --noEmit` clean, `eslint` clean, Vitest 90/90.
