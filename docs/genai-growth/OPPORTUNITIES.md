...
**Operational / Cost Risk:** Medium — image-generation calls cost money per call; needs a volume/cost bound (e.g., generate on a schedule, not on-demand per visitor) before scaling up.
...
**Recommended Timing:** Started 2026-08-06, running in parallel with Phase 1 Python work per Olga's decision — splitting working time between the two tracks rather than sequencing them.
