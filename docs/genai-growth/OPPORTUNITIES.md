...
**Update 2026-08-08 — (a)-(c) are actually already resolved in shipped code**, this section of the doc had gone stale relative to `trend-detection.ts`: (a) hardcoded to cross-stitch-tagged Pinterest, Etsy cross-stitch bestsellers, r/CrossStitch, cross-stitch-specific Google Trends, in `buildPrompt()`; (b) ~~the full live catalog album-caption list via `getAllAlbumCaptions()`, capped at `MAX_AVOID_LIST_SIZE=200`~~ **superseded 2026-08-08** — album-level captions turned out to be a real dedup gap (a "frog" subject can't be avoided via an avoid-list that only says "Children", the actual caption of the album holding the existing frog designs), so this is now unique individual DESIGN captions via `getExistingDesignCaptions()`, uncapped (2398 unique captions fits comfortably in context, ~7800 tokens); (c) shipped as `{theme, imagePrompt, signalSource, reasoning}`. Only (d) — run frequency/cost ceiling — remains genuinely open, still tied to the undecided trigger mechanism (button vs. scheduled). **Stopgap, not the real fix** — exact-string dedup still misses near-duplicates ("kawaii green frog" vs. "Kawaii Cottagecore Frog"); Olga wants to discuss embeddings/vector similarity for this next session (see `Focus.md`).

**Real fix built 2026-08-09 — the text avoid-list is gone, replaced by a `search_catalog` client-side tool.** Full design rationale in `DECISIONS.md` ADR-009. `buildPrompt()` no longer takes an `existingThemes` argument or lists any catalog captions at all — it just tells the model to call `search_catalog` before finalizing a theme. That tool (`runSearchCatalogTool()` in `trend-detection.ts`) runs a real embedding-similarity lookup (`findNearestTextMatch()`, new export in `semantic-search.ts`, reusing the already-shipped `embedText`/`loadVectorIndex`/`dotProduct` from Opportunity 2's semantic search) against the catalog's precomputed Titan text embeddings and returns the closest existing design's caption plus a raw similarity score — the model judges the score itself, no hardcoded threshold (no real score data exists yet to calibrate one against). This directly targets the gap the previous fix couldn't: comparison is by meaning, not spelling, so "kawaii green frog" should register as close to "Kawaii Cottagecore Frog" even with zero shared words. `detectTrend()`'s continuation loop extended to handle `tool_use` (client tool) alongside the existing `pause_turn` (server tool, `web_search`) handling; `MAX_CONTINUATIONS` raised 2→4 since both tool types now share the same turn budget. Verified: `tsc --noEmit` clean, `eslint` clean, Vitest 88/88 (13/13 in `trend-detection.test.ts`, updated for `buildPrompt()`'s new no-arg signature). **Not yet exercised by a real `detectTrend()` run** — next real run should confirm `search_catalog` actually gets called and show what similarity scores real near-duplicate vs. genuinely-new themes produce (no calibration data exists yet, see ADR-009 "Revisit When").
...
- **Resolved 2026-08-08: fixed `pattern-converter.ts` to accept real alpha-channel images directly**, not just worked around in the prompt. Triggered by a real Round 2 finding (`IMAGE_GENERATION_PREFERENCES.md`): what looked like a "vignette" on the OpenAI frog image was actually genuine `RGBA` transparency, and checking how `convertImage()` handled it surfaced a real, separate, more serious bug — `.removeAlpha()` doesn't composite onto any background, it just drops the alpha channel and keeps whatever RGB the encoder stored under transparent pixels (often black). Since `convertImage()` is called directly from the public `/api/convert` route with no pre-flatten step, **any real user uploading a PNG with genuine transparency got a black background in their pattern** — a live bug, not just a Track 2 concern. Fixed by decoding with `ensureAlpha()` + compositing onto white in `compositeOntoWhite()`, and tracking which cells were transparent so they become empty stitches (`-1`) rather than a spurious "white" palette color competing for a `maxColors` slot — no more border flood-fill guessing needed for images with real alpha. Verified against both the real transparent OpenAI frog (empty cells correctly excluded, palette has no phantom background color) and a real opaque image (zero behavior change: 0 empty cells, same palette shape as before). `next lint`/`tsc --noEmit` clean, Vitest 89/89 unchanged (no unit tests exist for `pattern-converter.ts` — verified via real converted images instead, matching this file's established practice). Cause B (single clear subject, clean color regions) is now the only real constraint left — the prompt's white-background language could in principle be relaxed for providers/cases that return real alpha, though the prompt itself hasn't been changed yet.
...
1. **Grounding — did it actually search, and is the finding real (not a stretch)? RESOLVED 2026-08-08, concrete mechanism designed, not yet implemented.** Only checked today via `hasRealWebSearchEvidence()` (a `server_tool_use` block exists) — proves *a* search happened, not that what it found is a strong signal vs. thin/marginal. It's the only one of the three that can be checked immediately, before spending money on image generation — Reach/Conversion below only resolve weeks later.

   **The fix needs no extra AI call — the data is already in the SDK response, unused.** Checked the actual Anthropic SDK types (`@anthropic-ai/sdk`'s `messages.d.ts`) rather than assuming: `ServerToolUseBlock.input` carries the real search query issued; `WebSearchToolResultBlock.content` (`WebSearchResultBlock[]`) carries the real `url`/`title`/`page_age` of what the search actually returned; and — the strongest signal — `TextBlock.citations` can contain `CitationsWebSearchResultLocation` entries (`url`, `title`, `cited_text`), which is specifically what Claude *cited* in its final answer, not just what it saw. All three are already present in every `detectTrend()` response today; nothing currently reads them.

   **Proposed deterministic check (code only, zero marginal cost):** extract `distinctQueries` (unique `ServerToolUseBlock.input.query` count), `distinctCitedUrls` and their domains (from `CitationsWebSearchResultLocation`), and each citation's `page_age`. Provisional gate: reject/flag-for-manual-review unless `distinctCitedUrls >= 2` **and** at least one citation's domain is in the cross-stitch-relevant allowlist (`pinterest.com`, `etsy.com`, `reddit.com`, `trends.google.com`) — catches both "only found one thin source" and "cited something real but off-topic, the niche-source instruction didn't actually bite." `page_age` is available for an optional staleness check (a "trending now" claim backed by a year-old page is weaker) but no threshold decided yet.

   Not yet implemented: `detectTrend()`/`extractJson()` don't currently expose citation data to the caller at all — needs a new exported helper (e.g. `assessGrounding(content: Anthropic.ContentBlock[])`) parallel to the existing `hasRealWebSearchEvidence()`, following the same pure/no-I/O/unit-testable pattern as `trend-detection.ts`'s other helpers.
