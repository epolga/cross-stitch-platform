...
| [pdf-structure.md](./pdf-structure.md) | Variant set `{1, 3, 5}` declared at `MainWindow.xaml.cs:85`; "main" PDF is bytes-identical to variant 1; PDFs come from `Converter.exe` (built from `uploader/Converter`, same monorepo, not part of `Uploader.sln`), not iTextSharp; partial-failure orphan risk; CloudFront-public, no PII | 12 sections, 39 citations |
...
| [publish-to-catalog-web.md](./publish-to-catalog-web.md) | F1b — the same design-publish contract, second implementation: a web-admin button in `/photo-to-cross-stitch` instead of the desktop Uploader. No `.scc` chart (editor's own grid/palette is the chart), no EB restart (in-process cache refresh instead — single-instance env only), no test mode. Built + first live design (5461 "Giraffes") verified 2026-08-04/05; found+fixed a `Description` field bug from the first live run | 12 sections, author-verified (not independently audited) |
