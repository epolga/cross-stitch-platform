...
describe('POST /api/converter/patterns', () => {
  ...
  it('saves pattern and returns id on happy path', async () => {
    ...
    expect(mocks.savePattern).toHaveBeenCalledWith(
      'Horse', 2, 1, VALID_BODY.palette, VALID_BODY.grid, 'u1', undefined, undefined, undefined, undefined, undefined, undefined,
    );
  });
  ...
  it('passes thumbnail to savePattern when provided', async () => {
    ...
    expect(mocks.savePattern).toHaveBeenCalledWith(
      'Horse', 2, 1, VALID_BODY.palette, VALID_BODY.grid, 'u1', thumb, undefined, undefined, undefined, undefined, undefined,
    );
  });
  ...
  it('passes researchImageKey to savePattern when provided', async () => {
    ...
    expect(mocks.savePattern).toHaveBeenCalledWith(
      ...
      'research-uploads/2026-08-10/abc.jpg', undefined, undefined,
    );
  });

  it('passes sourceImageKey to savePattern when provided', async () => {
    mocks.getSession.mockResolvedValueOnce({ userId: 'u1', email: 'a@b.com' });
    mocks.savePattern.mockResolvedValueOnce('new-pattern-id');

    await POST(makeRequest({ ...VALID_BODY, sourceImageKey: 'pattern-source-images/2026-08-11/abc.jpg' }));
    expect(mocks.savePattern).toHaveBeenCalledWith(
      'Horse', 2, 1, VALID_BODY.palette, VALID_BODY.grid, 'u1', undefined, undefined, undefined,
      undefined, 'pattern-source-images/2026-08-11/abc.jpg', undefined,
    );
  });

  it('passes sourceImageMaskKey to savePattern when provided', async () => {
    mocks.getSession.mockResolvedValueOnce({ userId: 'u1', email: 'a@b.com' });
    mocks.savePattern.mockResolvedValueOnce('new-pattern-id');

    await POST(makeRequest({
      ...VALID_BODY,
      sourceImageKey: 'pattern-source-images/2026-08-11/abc.jpg',
      sourceImageMaskKey: 'pattern-source-images/2026-08-11/abc.mask.png',
    }));
    expect(mocks.savePattern).toHaveBeenCalledWith(
      'Horse', 2, 1, VALID_BODY.palette, VALID_BODY.grid, 'u1', undefined, undefined, undefined,
      undefined, 'pattern-source-images/2026-08-11/abc.jpg', 'pattern-source-images/2026-08-11/abc.mask.png',
    );
  });
});
