...
//   1. POST with no body (or {} ) — server diffs the AI-generated snapshot
//      against the pattern's current saved state. An empty diff (the
//      "Approve, no changes" case) finalizes immediately. A non-empty diff
//      is returned WITHOUT being persisted yet, for the UI to show and ask
//      "what were you fixing" — finalized: false tells the client to call
//      again.
...
export async function POST(
  ...
  try {
    ...
    if (isEmptyDiff(diff) || reasonTags !== undefined) {
      ...
      const result = await submitReview(pattern.sourceGenerationId, diff, reasonTags ?? [], freeTextComment);
